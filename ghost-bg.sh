...
MAX_OFFSET_FILE="/tmp/.ghost-maxoff-$TAG"
MAX_OFFSET_LOCK="/tmp/.ghost-maxoff-lock-$TAG"
: > "$MAX_OFFSET_FILE"
...
for bin in xterm alacritty wmctrl xdotool xev xwininfo xrandr zcat; do
...
# tamaño de UN solo monitor (no el total), lo que haría que el clamp de
...
# Arranca en la misma posición nominal que la de adelante;
# measure_normal_offset la corrige apenas ambas ventanas están mapeadas.
xterm -title "$BACK_TITLE" -bg black -geometry "+$START_X+$START_Y" -e "$SELF_DIR/ghost-ansi.sh" &
BACK_PID=$!
...
# de teclado (salida a /dev/null), cualquier aviso por stderr se pierde para
# siempre. Este log queda en disco pase lo que pase, para poder auditar una
# calibración rara después de los hechos.
DEBUG_LOG="/tmp/ghost-bg-debug.log"
...
# Deliberadamente NO se usa "xdotool getwindowgeometry" para esto: para una
# ventana con frame, xdotool puede devolver una traducción de coordenadas
# que ya trae este mismo inset sumado una vez de más (verificado a mano
# comparando ambas herramientas sobre la misma ventana) — usarlo para
# calibrar termina sumando el inset dos veces, empujando el fondo hacia la
...
reposition() {
    ...
    # La de adelante puede quedar parcialmente (o del todo) fuera de la
    # pantalla si se la arrastra a un borde. Lo único que controlamos es la
    # de atrás: la encajamos para que nunca dibuje fuera del escritorio
    # virtual (SCREEN_W/SCREEN_H, calculados una sola vez al inicio). Si se
    # pasa de CUALQUIER borde, recortar el TAMAÑO correspondiente y nunca
    # "correr" la esquina opuesta: la posición es la que mantiene alineada
    # la esquina de la cuadrícula de texto con la de adelante (ver
    # cmatrix-bg.sh para el detalle de por qué mover en vez de recortar
    # desalinea la esquina superior/izquierda).
